...
import { Heart, MessageSquare, Share2, ChevronUp, ChevronDown, X, Volume2, VolumeX, Play, Pause, Flag, Ban } from "lucide-react";
...
const VideoPlayer = ({ videos, startIndex = 0, onClose }: VideoPlayerProps) => {
  ...
  useEffect(() => {
    ...
    const fetchAcks = async () => {
      const { data } = await supabase.from("video_acknowledges").select("video_id").eq("user_id", user.id);
  ...
  return (
    ...
    >
      ...
      {/* Video container */}
      <div
        className="relative w-full h-full max-w-[450px] max-h-[100vh] mx-auto flex items-center justify-center"
      ...
      <AnimatePresence>
        {showComments && (
          <motion.div
            ...
            className="absolute bottom-0 left-0 right-0 bg-card rounded-t-2xl max-h-[60vh] overflow-hidden z-50"
